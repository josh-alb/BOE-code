...
/* _Important comment about how pitches and keys are represented_
  ...
  Notes may be inputted in either Note format (with just a pitch) or TPCNote format (with a
  TPC). The user may also specify "npc_or_tpc_profile": 0 for npc, 1 for tpc. (A TPC profile is like
  an NPC profile except that a user-settable default value is used for TPC's outside the range of b2
  to #4.) We assume that Note format will be used with an npc profile; if not, a fatal error is
  reported. TPCNote format could be used with either; if you use TPCNote input with an npc profile,
  the notes are simply mapped on to one cycle of the LOF.  (Scoring mode 0 or 3 requires an npc
  profile; otherwise a fatal error is reported.) In short: Note input with a TPC profile is
  forbidden.

  Some scoring algorithms also require NPC profile, namely the K-S algorithm (scoring_mode = 0) and
  the Bayesian algorithm (scoring_mode = 1). If one of these is used with a TPC profile, it's a
  fatal error.

  Pitches (e.g. note[].tpc values) are always represented in line-of-fifths order, with C =
  14. (This is done when the pitches are first read in). (If there's Note input, or TPCNote input
  and npc_or_tpc_profile = 0, all pitches are shifted into one cycle of the line of fifths, from 9
  to 20 inclusive; again, this is done when the input is read.)

  Keys are also represented in line-of-fifths order. Major keys are 0 to 27, C=14; minor keys are
  28-55, C minor = 42. If npc_or_tpc_profile = 0, the search is nominally done on all keys, but only
  keys from 9 to 20 (inclusive) and 37 to 48 (inclusive) are looked at; others are given large
  negative values.

  Although key-profiles are read in in pitch-height order, they are then adjusted (in
  generate_tpc/npc_profiles) to line-of-fifths order, with the tonic as 5. */
...
void create_chords() {                                   /* Now we create the chords; each chord starts and ends at a beat of 
...
void create_segments() {                                 /* Each segment starts at a sbeat and ends at the following sbeat */
    int b, s;
...
void count_segment_notes() {                             /* In each segment, tally up the notes of each TPC */
    int s, y, n;
    double total_dur;
