...
/* This is a polyphonic key-finding program. It takes input either in the form of "Note" statements
(above) or "TPCNote" statements (below):

Note [ontime] [offtime] [pitch] 
TPCNote [ontime] [offtime] [pitch] [TPC]

where ontime and offtime are in milliseconds, pitch is in integer notation (middle C = 60), and TPC
is in "line-of-fifths" pitch-class notation (Gb=-4...C=2, G=3, D=4...F#=6, etc.).

The program also requires "beat" statements of the form "Beat [time] [level]".  It uses these to
divide the piece into short segments (generally segments are defined by level 3 or level 4
beats). It then does a key analysis in which every segment is assigned a key. The program can also
take "Chord" statements ("Chord [ontime] [offtime] [root]", where "root" is a TPC) and use these,
combined with the key analysis, to make a Roman numeral analysis.

To compile the program, type "make". Then to run it, type "./key [input-file]".

The program can be used to implement several different key-finding algorithms: the
Krumhansl-Schmuckler algorithm, Temperley's algorithm in _CBMS_, and Temperley's polyphonic Bayesian
algorithm (see _Music and Probability_, chapter 6, for explanation).

With verbosity = 1, the program outputs the key analysis, giving a key symbol for each segment. With
verbosity = 2 or 3, it outputs a lot more information as well.

A parameter file contains a number of parameters for the program (the default file is
"parameters"). These include key-profile values, the scoring algorithm used (0=K-S algorithm, 1=CBMS
algorithm, 3=Bayesian algorithm), and other things.  See "parameters" for comments.

The flag "-p" followed by a filename specifies the parameter file. The flag "-s", followed by an
integer, specifies the metrical level to be used for segmenting the piece.

Keys are internally represented with line of fiths numbers: G=13, C=14, F=15...Gm=41, Cm=42,
Fm=43...etc.
